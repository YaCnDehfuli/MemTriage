...
export function RiskSummaryPanel({ summary }: { summary: RiskSummary | null }) {
  ...
  return (
    <Panel eyebrow="Posture" title="Risk summary">
      <div className="grid grid-cols-4 gap-px overflow-hidden rounded-b-lg bg-surface-700/40">
        {RISK_ORDER.map((r) => (
          <div key={r} className="bg-surface-850 px-3 py-4 text-center">
            <div
            ...
            <div className="mt-1 text-[10px] uppercase tracking-wider text-ink-400">{r}</div>
          </div>
        ))}
      </div>
      <div className="px-4 py-2 text-center text-[11px] text-ink-400">
...
export function AttackPanel({ techniques }: { techniques: AttackTechnique[] }) {
  return (
    <Panel eyebrow="Framework alignment" title="MITRE ATT&CK">
      {techniques.length === 0 ? (
        <div className="px-4 py-6 text-center text-xs text-ink-400">No techniques aligned.</div>
      ) : (
        <ul className="divide-y divide-surface-800/70">
          {techniques.map((t) => (
            <li key={t.technique_id} className="flex items-start gap-3 px-4 py-2.5">
              <span className="mt-0.5 rounded bg-surface-800 px-1.5 py-0.5 font-mono text-[11px] text-accent-soft ring-1 ring-inset ring-surface-600">
                {t.technique_id}
              </span>
              <div className="min-w-0 flex-1">
                <div className="text-[13px] font-medium text-ink-100">{t.name}</div>
                <div className="text-[11px] text-ink-400">{t.tactic}</div>
              </div>
